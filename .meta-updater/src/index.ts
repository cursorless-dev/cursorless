...
export default async (workspaceDir: string) => {
  const pnpmManifest = loadJsonFile.sync<any>(
    path.join(workspaceDir, "pnpm/package.json"),
  );
  ...
  return createUpdateOptions({
    "package.json": (
      manifest: (ProjectManifest & { keywords?: string[] }) | null,
      { dir },
    ) => {
      if (!manifest) {
        return manifest;
      ...
      if (manifest.name === "monorepo-root") {
        manifest.scripts![
          "release"
        ] = `pnpm --filter=@pnpm/exe publish --tag=${NEXT_TAG} --access=public && pnpm publish --filter=!pnpm --filter=!@pnpm/exe --access=public && pnpm publish --filter=pnpm --tag=${NEXT_TAG} --access=public`;
        return manifest;
      }
      ...
      manifest.keywords = [
        pnpmMajorKeyword,
        ...(manifest.keywords ?? []).filter(
          (keyword) => !/^pnpm[0-9]+$/.test(keyword),
        ),
      ];
...
async function updateTSConfig(
  ...
  { dir, manifest }: FormatPluginFnOptions,
) {
  ...
  await writeJsonFile(
    path.join(dir, "tsconfig.lint.json"),
    {
      ...
      include: ["src/**/*.ts", "test/**/*.ts", "../../__typings__/**/*.d.ts"],
    },
    { indent: 2 },
  );
  return {
...
async function updateManifest(
  ...
  if (manifest.name === CLI_PKG_NAME) {
    ...
    scripts.compile +=
      " && rimraf dist bin/nodes && pnpm run bundle \
