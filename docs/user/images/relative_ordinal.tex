...
\newcommand{\funk}[3][black]{
    \draw[fill=#1] (0,-#2*\vspacing) circle (0.1cm) node[left=3pt,align=right]
    {\texttt{"#3"}};
}
...
\draw[fill=red] (-4.1cm,-11.5*\vspacing) circle (0.1cm) node[right=3pt,align=left] {= input target};

% \abrace [aspect], horizontal offset, start funk, end funk, text
\abrace[0.75]{0}{6}{8}{\texttt{"next three funks"}};
\abrace[0.25]{0}{2}{4}{\texttt{"previous three funks"}};
...
\abrace{13.5}{0}{10}{iteration scope};
