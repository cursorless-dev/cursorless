...
[existing test cases](../../src/packages/cursorless-vscode-e2e/suite/fixtures/recorded) for example outputs.
...
1. Create a minimal file to use for recording tests. And position your cursor
   ...
   [existing test cases](../../src/packages/cursorless-vscode-e2e/suite/fixtures/recorded) for examples.
1. Issue the `"cursorless record"` command. Alternately, issue one of the special recording commands listed in
   - List of target directories is shown. All test cases will be put into the
     given subdirectory of `src/packages/cursorless-vscode-e2e/suite/fixtures/recorded`
...
1. Change the value at the `custom` key in `AVAILABLE_TRANSFORMATIONS` at the top of
   [`transformRecordedTests/index.ts`](../../src/packages/scripts/transformRecordedTests/index.ts) to
