...
import { ScopeType, SelectionContext, SelectionWithContext } from "./Types";
...
const nodeMatchers: Record<ScopeType, NodeMatcher> = {
  ...
  value(editor: TextEditor, node: SyntaxNode) {
    ...
    const leadingDelimiterRange = new Range(
      new Position(
        leadingDelimiterToken.startPosition.row,
        leadingDelimiterToken.startPosition.column
      ),
      new Position(valueNode.startPosition.row, valueNode.startPosition.column)
    );
