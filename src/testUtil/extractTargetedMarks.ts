...
import { PrimitiveTargetDesc, TargetDesc } from "../typings/target.types";

function extractPrimitiveTargetKeys(...targets: PrimitiveTargetDesc[]) {
  const keys: string[] = [];
...
export function extractTargetKeys(target: TargetDesc): string[] {
  switch (target.type) {
    case "primitive":
