 ...
 */
export function rangesToRangeInfos(
  document: TextDocument,
...
function fillOutRangeInfos(
  document: TextDocument,
  rangeInfoMatrix: RangeInfo[][]
): rangeInfoMatrix is FullRangeInfo[][] {
  rangeInfoMatrix.forEach((rangeInfos) =>
    rangeInfos.map((rangeInfo) => {
      const { range } = rangeInfo;
      Object.assign(rangeInfo, {
        offsets: {
          start: document.offsetAt(range.start),
          end: document.offsetAt(range.end),
        },
...
export async function performEditsAndUpdateSelectionInfos(
  ...
  editor: TextEditor,
  edits: Edit[],
  originalRangeInfos: RangeInfo[][]
) {
  fillOutRangeInfos(editor.document, originalRangeInfos);

  return await performEditsAndUpdateFullSelectionInfos(
    rangeUpdater,
    editor,
    edits,
    originalRangeInfos as FullRangeInfo[][]
  );
}
