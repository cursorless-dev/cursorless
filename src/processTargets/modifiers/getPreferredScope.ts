...
export function getPreferredScope<T extends Scope>(scopes: T[]): T {
  return getPreferredScopeHelper(
    scopes,
    (scope1, scope2) =>
...
export function getLeftScope<T extends Scope>(scopes: T[]): T {
  return getPreferredScopeHelper(scopes, (scope1, scope2) =>
    scope1.domain.start.isBefore(scope2.domain.start)
  );
}

export function getRightScope<T extends Scope>(scopes: T[]): T {
  return getPreferredScopeHelper(scopes, (scope1, scope2) =>
    scope1.domain.start.isAfter(scope2.domain.start)
  );
}

function getPreferredScopeHelper<T extends Scope>(
  scopes: T[],
